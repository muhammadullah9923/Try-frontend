...
export default function VendorEarnings({ vendorId }) {
  ...
  const fetchEarnings = async () => {
    setLoading(true);
    try {
      const res = await fetch(`http://100.48.58.109:8080/api/vendor/${vendorId}/earnings/?period=${period}`, {
        credentials: "include"
      });
  ...
  const fetchPayouts = async () => {
    try {
      const res = await fetch(`http://100.48.58.109:8080/api/vendor/${vendorId}/payouts/`, {
        credentials: "include"
      });
      const data = await res.json();
      if (data.success) {
