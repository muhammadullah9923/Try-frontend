...
export default function VendorProductList({ vendorId }) {
  ...
  useEffect(() => {
    if (!vendorId) return;
    setLoading(true);
    fetch(`http://100.48.58.109:8080/api/vendor/${vendorId}/products/`)
      .then(r => r.json())
      .then(data => {
          ...
          <tbody>
            {products.map(p => (
              <tr key={p.id}>
                ...
                <td>{p.occasion || "-"}</td>
                <td>{p.description}</td>
                <td>{p.image ? <img src={p.image.startsWith('http') ? p.image : `http://100.48.58.109:8080${p.image}`} alt={p.name} style={{ width: 60, borderRadius: 6 }} /> : "-"}</td>
