...
export default function Recommendations({ user }) {
    ...
    useEffect(() => {
        ...
        fetch(`http://100.48.58.109:8080/api/recommendations/${occasionId}/`)
            .then(res => res.json())
            .then(data => {
                if (data.success) {
                ...
                {products.map(product => (
                    <div key={product.id} style={{
                        ...
                        <div style={{ position: 'relative', height: '220px', marginBottom: '1rem' }}>
                            <img src={product.image ? (product.image.startsWith('http') ? product.image : `http://100.48.58.109:8080${product.image}`) : ''}
                                alt={product.name}
