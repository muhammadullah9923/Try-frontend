...
export default function VendorProducts({ vendorId, onStatsChange }) {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const res = await fetch(`http://100.48.58.109:8080/api/vendor/${vendorId}/products/`);
  ...
  const handleAddProduct = async (e) => {
    ...
    try {
      const res = await fetch("http://100.48.58.109:8080/api/vendor/add_product/", {
        method: "POST",
        body: data
      });
  ...
  const handleEditProduct = async (e) => {
    ...
    try {
      const res = await fetch(`http://100.48.58.109:8080/api/vendor/edit_product/${editingProduct.id}/`, {
        method: "POST",
        body: data
      });
      const result = await res.json();
  ...
  const handleDeleteProduct = async (productId) => {
    ...
    try {
      const res = await fetch(`http://100.48.58.109:8080/api/vendor/delete_product/${productId}/`, {
        method: "POST",
        credentials: "include"
      });
      const data = await res.json();
  ...
  const handleToggleStatus = async (productId) => {
    try {
      const res = await fetch(`http://100.48.58.109:8080/api/vendor/product/${productId}/toggle_status/`, {
        method: "POST",
        credentials: "include"
      });
      const data = await res.json();
  ...
  const handleUpdateStock = async (productId, newStock) => {
    try {
      const res = await fetch(`http://100.48.58.109:8080/api/vendor/product/${productId}/update_stock/`, {
        method: "POST",
              ...
              {filteredProducts.map((product) => (
                <tr key={product.id} style={{ 
                  ...
                  <td style={{ padding: 12 }}>
                    {product.image ? (
                      <img
                        src={product.image.startsWith('http') ? product.image : `http://100.48.58.109:8080${product.image}`}
                        alt={product.name}
